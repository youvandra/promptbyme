...
export const ProjectSpacePage: React.FC = () => {
 ...
 const InputNode = ({ data }: NodeProps) => (
  <div className="px-4 py-3 shadow-md rounded-lg bg-purple-600/20 border border-purple-500/30 min-w-[200px] max-w-[300px] hover:bg-purple-600/30 hover:border-purple-500/50 transition-all duration-200">
    <div className="font-bold text-sm text-purple-300 mb-2">{data.label}</div>
     {data.content && (
      <div className="text-xs text-purple-200 bg-black/20 p-2 rounded border border-purple-500/20 overflow-hidden max-h-[100px] overflow-y-auto">{data.content}</div>
     )}
   </div>
 )

 const PromptNode = ({ data }: NodeProps) => (
  <div className="px-4 py-3 shadow-md rounded-lg bg-blue-600/20 border border-blue-500/30 min-w-[200px] max-w-[300px] hover:bg-blue-600/30 hover:border-blue-500/50 transition-all duration-200">
    <div className="font-bold text-sm text-blue-300 mb-2">{data.label}</div>
     {data.content && (
      <div className="text-xs text-blue-200 bg-black/20 p-2 rounded border border-blue-500/20 overflow-hidden max-h-[100px] overflow-y-auto">{data.content}</div>
     )}
   </div>
 )

 const ConditionNode = ({ data }: NodeProps) => (
  <div className="px-4 py-3 shadow-md rounded-lg bg-yellow-600/20 border border-yellow-500/30 min-w-[200px] max-w-[300px] hover:bg-yellow-600/30 hover:border-yellow-500/50 transition-all duration-200">
    <div className="font-bold text-sm text-yellow-300 mb-2">{data.label}</div>
     {data.content && (
      <div className="text-xs text-yellow-200 bg-black/20 p-2 rounded border border-yellow-500/20 overflow-hidden max-h-[100px] overflow-y-auto">{data.content}</div>
     )}
   </div>
 )

 const OutputNode = ({ data }: NodeProps) => (
  <div className="px-4 py-3 shadow-md rounded-lg bg-green-600/20 border border-green-500/30 min-w-[200px] max-w-[300px] hover:bg-green-600/30 hover:border-green-500/50 transition-all duration-200">
    <div className="font-bold text-sm text-green-300 mb-2">{data.label}</div>
     {data.content && (
      <div className="text-xs text-green-200 bg-black/20 p-2 rounded border border-green-500/20 overflow-hidden max-h-[100px] overflow-y-auto">{data.content}</div>
     )}
   </div>
 )
