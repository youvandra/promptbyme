...
export const ApiDocsModal: React.FC<ApiDocsModalProps> = ({
  ...
  const [expandedSections, setExpandedSections] = useState<Set<string>>(new Set(['authentication', 'run-prompt', 'run-flow']))
  ...
  "error": "Error message details."
            ...
            <div className="border border-zinc-800/50 rounded-xl overflow-hidden">
              <button
                ...
                className="w-full flex items-center justify-between p-4 text-left bg-zinc-800/30 hover:bg-zinc-800/50 transition-colors"
              >
                <h3 className="text-lg font-semibold text-white">Run Prompt Flow API</h3>
                {expandedSections.has('run-flow') ? (
                  <ChevronDown size={20} className="text-zinc-400" />
                ) : (
                  <ChevronRight size={20} className="text-zinc-400" />
                )}
              ...
              <AnimatePresence>
                {expandedSections.has('run-flow') && (
                  ...
                  >
                    <div className="p-4 border-t border-zinc-800/50">
                      <p className="text-zinc-300 mb-4">
                        This endpoint allows you to execute a complete prompt flow with optional variable substitution.
                      </p>
                      ...
                      <div className="bg-zinc-800/50 p-4 rounded-lg mb-4">
                        <div className="flex items-center justify-between mb-2">
                          <span className="text-sm text-zinc-400">Endpoint</span>
                          <button
                            onClick={() => copyToClipboard(`POST ${import.meta.env.VITE_SUPABASE_URL}/functions/v1/run-prompt-flow-api`, 'flow-endpoint')}
                            className="p-1 text-zinc-400 hover:text-white hover:bg-zinc-700/50 rounded transition-colors"
                            title="Copy to clipboard"
                          >
                            {copied === 'flow-endpoint' ? (
                              <CheckCircle size={16} className="text-emerald-400" />
                            ) : (
                              <Copy size={16} />
                            )}
                        ...
                        <pre className="text-sm text-indigo-300 font-mono">
                          <code>POST {import.meta.env.VITE_SUPABASE_URL}/functions/v1/run-prompt-flow-api</code>
                        </pre>
                      </div>
  ...
  "max_tokens": 1000
                      ...
                      <table className="w-full text-sm text-left text-zinc-300 mb-4">
                        ...
                        <tbody>
                          <tr className="border-b border-zinc-800">
                            <td className="px-4 py-3 font-medium">flow_id</td>
                            <td className="px-4 py-3">string</td>
                            <td className="px-4 py-3">Yes</td>
                            <td className="px-4 py-3">The UUID of the flow to run</td>
                          </tr>
                          <tr className="border-b border-zinc-800">
                            <td className="px-4 py-3 font-medium">variables</td>
                            <td className="px-4 py-3">object</td>
                            <td className="px-4 py-3">No</td>
                            <td className="px-4 py-3">Key-value pairs for variable substitution across all steps in the flow</td>
                          </tr>
                          <tr className="border-b border-zinc-800">
                            <td className="px-4 py-3 font-medium">api_key</td>
                            <td className="px-4 py-3">string</td>
  ...
  if (data.success) {
    ...
    console.error('Error:', data.error);
                      ...
                      <h4 className="text-md font-semibold text-white mb-2">JavaScript/TypeScript for Running a Flow</h4>
                      
                      <div className="bg-zinc-800/50 p-4 rounded-lg mb-4">
                        <div className="flex items-center justify-between mb-2">
                          <span className="text-sm text-zinc-400">Example</span>
                          <button
                            onClick={() => copyToClipboard(`async function runFlow() {
...
`}</code>
                      ...
                      <h4 className="text-md font-semibold text-white mb-2 mt-4">Python for Running a Flow</h4>
...
def run_flow():
    ...
    if data.get('success'):
        ...
        print('Error:', data.get('error'))`, 'python-flow-example')}
                            className="p-1 text-zinc-400 hover:text-white hover:bg-zinc-700/50 rounded transition-colors"
                            title="Copy to clipboard"
                          >
                            {copied === 'python-flow-example' ? (
                              <CheckCircle size={16} className="text-emerald-400" />
                            ) : (
                              <Copy size={16} />
                            )}
