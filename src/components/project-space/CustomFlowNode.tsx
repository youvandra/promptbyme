...
import { Edit3, GitBranch, Target, Upload, Trash2, Maximize2, User } from 'lucide-react'
...
const CustomFlowNode: React.FC<NodeProps> = ({ id, data, selected }) => {
  ...
  // Check if node is assigned to current user
  const isAssignedToCurrentUser = data.nodeData?.metadata?.assignTo === currentUserId
    ...
    <div className="relative">
      ...
      <motion.div 
        className={`w-[250px] h-[150px] ${getNodeColor()} backdrop-blur-sm border rounded-xl p-4 shadow-lg flex flex-col items-start ${
          ...
          isAssignedToCurrentUser && !selected ? 'ring-2 ring-indigo-400/50 shadow-lg shadow-indigo-500/30 animate-pulse' : ''
        }`}
      ...
      <Handle
        ...
        className={`w-3 h-3 bg-zinc-700 border-2 border-white cursor-pointer !opacity-100 ${
          isAssignedToCurrentUser ? 'bg-indigo-500' : ''
        }`}
      />
    </div>
  )
