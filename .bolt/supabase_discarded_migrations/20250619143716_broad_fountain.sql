...
CREATE POLICY "project_members_update" ON project_members
  ...
  WITH CHECK (
    ...
    (user_id = auth.uid() AND OLD.role = NEW.role AND OLD.invited_by_user_id = NEW.invited_by_user_id)
    OR
    -- Admins can update roles but not change core relationships
    (
      EXISTS (
        SELECT 1 FROM project_members pm
        WHERE pm.project_id = project_members.project_id
      ...
      AND OLD.invited_by_user_id = NEW.invited_by_user_id
    )
  );
...
-- Create function to add project owner as admin when project is created
