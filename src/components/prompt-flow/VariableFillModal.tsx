...
import { X, Wand2, CheckCircle, Eye } from 'lucide-react'
...
interface VariableFillModalProps {
  ...
  onClose: () => void
  step: FlowStep
  onVariablesFilled: (stepId: string, filledContent: string, variables: Record<string, string>) => void
}
...
export const VariableFillModal: React.FC<VariableFillModalProps> = ({
  ...
  const [currentStep, setCurrentStep] = useState<'fill' | 'preview'>('fill')

  // Extract variables from prompt content
  useEffect(() => {
    if (isOpen && step) {
      const content = step.prompt_content || ''
      const variableMatches = content.match(/\{\{([^}]+)\}\}/g) || []
      const uniqueVariables = Array.from(new Set(variableMatches))
        .map(match => {
          const name = match.replace(/[{}]/g, '')
          // Check if we have a saved value for this variable
          const savedValue = step.variables && step.variables[name] ? step.variables[name] : ''
  ...
  const generateFilledPrompt = () => {
    let filledPrompt = step.prompt_content || ''
  ...
  const handleSaveVariables = () => {
    ...
    onVariablesFilled(step.id, filledPrompt, variablesObj)
    onClose()
  }
  ...
  const renderLivePreview = () => {
    let highlightedPrompt = step.prompt_content || ''
      ...
      >
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-lg sm:text-xl font-semibold text-white">
                {currentStep === 'fill' ? 'Fill Variables' : 'Preview & Save'}
              </h2>
        ...
        <div className="flex-1 overflow-y-auto">
          <AnimatePresence mode="wait">
            {/* Step 1: Fill variables with live preview */}
            {currentStep === 'fill' && (
              <motion.div
                key="fill"
              ...
              >
                <div className="text-center mb-6">
                  ...
                  <p className="text-zinc-400">
                    Review the filled variables and save when ready
                  </p>
                </div>
                ...
                <div className="flex flex-col sm:flex-row gap-3 pt-4">
                  ...
                  >
                    <CheckCircle size={16} />
                    <span>Save Customized Prompt</span>
                  </button>
                </div>
              </motion.div>
            )}
