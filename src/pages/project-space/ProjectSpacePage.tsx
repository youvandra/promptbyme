...
export const ProjectSpacePage: React.FC = () => {
  ...
  const isViewingProject = location.pathname.startsWith('/project/') && location.pathname !== '/project-space'
  ...
  const renderProjectSpace = () => {
    ...
    return (
      <div className="h-full flex flex-col">
        {/* Project Header */}
        <div className="flex items-center justify-between mb-6 bg-zinc-900/50 border border-zinc-800/50 rounded-xl p-4">
          ...
          <div className="flex items-center gap-3">
            <div className="flex items-center gap-2 text-sm">
              <div className={`flex items-center gap-1 ${
                ...
                <span>{getVisibilityText(currentProject.visibility)}</span>
              </div>
            </div>
        ...
        {/* Project Content */}
        <div className="flex-1 bg-zinc-900/30 border border-zinc-800/50 rounded-xl p-6 overflow-auto">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {currentProject.nodes.map(node => (
                <div
                  ...
                  className={`bg-zinc-800/50 border border-zinc-700/50 rounded-xl p-4 cursor-pointer hover:border-${
                    node.type === 'input' ? 'purple' : 
                    node.type === 'prompt' ? 'blue' : 
                    node.type === 'condition' ? 'yellow' : 'green'
                  }-500/50 transition-all duration-200 transform hover:scale-[1.02]`}
                >
                  <div className="flex items-center gap-2 mb-3">
                    <div className={`p-2 bg-${
                      ...
                      node.type === 'prompt' ? 'blue' : 
                      node.type === 'condition' ? 'yellow' : 'green'
                    }-500/20 rounded-lg text-${
                      node.type === 'input' ? 'purple' : 
                      node.type === 'prompt' ? 'blue' : 
                      node.type === 'condition' ? 'yellow' : 'green'
                    }-400`}>
                      {node.type === 'input' && <Upload size={16} />}
                      {node.type === 'prompt' && <Edit3 size={16} />}
                      {node.type === 'condition' && <GitBranch size={16} />}
                      {node.type === 'output' && <Target size={16} />}
                    </div>
                    <h3 className="text-white font-medium text-sm truncate">{node.title}</h3>
                  </div>
                  
                  <div className="text-zinc-400 text-xs line-clamp-3 mb-3">
              ...
              <div className="text-center">
                ...
                <button
                  onClick={() => {
                    ...
                    // This is a placeholder - you would implement actual node creation
                    alert('Node creation would go here');
                  }}
    ...
    <>
      ...
      {selectedNode && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          <div className="absolute inset-0 bg-black/60 backdrop-blur-sm" onClick={() => setShowNodeDetails(false)} />
          
          <div className="relative bg-zinc-900/95 backdrop-blur-xl border border-zinc-800/50 rounded-2xl w-full max-w-2xl max-h-[90vh] overflow-hidden flex flex-col">
            {/* Header */}
            <div className="flex items-center justify-between p-6 border-b border-zinc-800/50">
              <div className="flex items-center gap-3">
                <div className={`p-2.5 bg-${
                  selectedNode.type === 'input' ? 'purple' : 
                  selectedNode.type === 'prompt' ? 'blue' : 
                  selectedNode.type === 'condition' ? 'yellow' : 'green'
                }-500/20 rounded-lg text-${
                  selectedNode.type === 'input' ? 'purple' : 
                  selectedNode.type === 'prompt' ? 'blue' : 
                  selectedNode.type === 'condition' ? 'yellow' : 'green'
                }-400`}>
                  {selectedNode.type === 'input' && <Upload size={20} />}
                  {selectedNode.type === 'prompt' && <Edit3 size={20} />}
                  {selectedNode.type === 'condition' && <GitBranch size={20} />}
                  {selectedNode.type === 'output' && <Target size={20} />}
                </div>
                <div>
            ...
            <div className="flex-1 overflow-y-auto p-6">
              <div className="bg-zinc-800/30 border border-zinc-700/30 rounded-xl p-4 mb-4">
                ...
                <div className="text-zinc-200 text-sm whitespace-pre-wrap">
                  {selectedNode.content || 'No content defined'}
                </div>
              </div>
              
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <h3 className="text-sm font-medium text-zinc-300 mb-2">Created</h3>
                ...
                <div>
                  <h3 className="text-sm font-medium text-zinc-300 mb-2">Updated</h3>
                  <div className="bg-zinc-800/30 border border-zinc-700/30 rounded-xl p-3">
                    <p className="text-zinc-200 text-sm">
            ...
            <div className="p-6 border-t border-zinc-800/50 bg-zinc-900/30">
              <div className="flex items-center justify-between">
                ...
                <button
                  onClick={() => {
                    // Edit node functionality would go here
