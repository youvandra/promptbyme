/*
  # Create prompt_flow_step table for customized prompt content in flows

  1. New Table
    - `prompt_flow_step` - Stores customized content for flow steps
      - `id` (uuid, primary key)
      - `flow_step_id` (uuid, references flow_steps)
      - `custom_content` (text, the modified content with filled variables)
      - `variables` (jsonb, stores variable values used in this step)
      - `created_at` (timestamptz)
      - `updated_at` (timestamptz)

  2. Security
    - Enable RLS on the table
    - Add policies for authenticated users to manage their own flow step content
    - Ensure proper cascading on deletion
